...
dir_prefix='/Users/rselin/Documents/NSF_hail/streamlined_hail_scripts/right-mover/lauren_new'
eps='300'
minlns='9'
short_file_prefix = 'cntl_Wrelative_fgt45mm'
label = 'CNTL run; Hailstones >= 45 mm'
subtitle = 'Kingfisher'
...
if dir_prefix.find('left') >= 0:
    supercluster_choices = {'A': ('14','19'), 'B': ('15',),
                            ...
                            'K': ('4',), 'L': ('7','8'), 'M': ('56',)}
elif dir_prefix.find('lauren_new') >= 0:
    supercluster_choices = {'A': ('12','18','19','21','22','24','26','27','36','40','41','49','50'),
                            'B': ('120','121','122','124','125'),
                            'C': ('119','125','126'),
                            'D': ('32',),
                            'E': ('5','8'),
                            'F': ('6','7','15'),
                            'G': ('10','11','38','63','64','66','70','72','87','100','101','108','110'),
                            'H': ('65','69','88'),
                            'I': ('67','68','71','75','76','79','80','81','82','83','86','102','105','114'),
                            'J': ('74','89','107','111','131'),
                            'K': ('2','51'),
                            'L': ('106','115'),
                            'M': ('73','85','109'),
...
for count, sc_name in enumerate(supercluster_names[0:num_sc_choices]):
    ...
    for i, cluster in enumerate(clusters):
        ...
        if first_time_through == 1:
            ...
            supersx = np.append(supersx, info['sx'])
            superex = np.append(superex, info['ex'])
            supersy = np.append(supersy, info['sy'])
            superey = np.append(superey, info['ey'])
            supersz = np.append(supersz, info['sz'])
            superez = np.append(superez, info['ez'])
            sc_mass_rate = np.append(sc_mass_rate, mass_rate)
...
for count, sc_name in enumerate(supercluster_names[0:num_sc_choices]):
    print ('Plotting Supercluster '+sc_name)

    if dir_prefix.find('4d') >= 0:
        if dir_prefix.find('left') >= 0:
            if np.isin(sc_name, np.array(['A','B','D','E','G','L'])):
                numxs = 100
                numys = 100
                xmin = -10000
                ymin = -20000
            elif np.isin(sc_name, np.array(['C','F','H','I','J','M'])):
                numxs = 120
                numys = 120
                xmin = -5000
                ymin = -25000
    ...
    for i, (x, y, z) in enumerate(zip(allx, ally, allz)):
        ...
        if lastz < 10:
            ax.plot(x,y,'o',color=sc_colors[count])
        lastz = z[-1]
    ax.set_xlabel('X (km)')
    ax.set_ylabel('Y (km)')
    ...
    for i, (x, y) in enumerate(zip(allx, allz)):
        ...
        if lastz < 10:
            bx.plot(x,y,'o',color=sc_colors[count])
        lastz = y[-1]
    bx.set_ylabel('Z (km)')
    ...
    for i, (x, y) in enumerate(zip(allz, ally)):
        ...
        if lastz < 10:
            cx.plot(x,y,'o',color=sc_colors[count])
        lastz = x[-1]
    cx.set_xlabel('Z (km)')
    ...
    fig.suptitle(label+', Supercluster '+sc_name+'\n'+
        'Eps: '+eps+'  MinLns: '+minlns,multialignment='center')
    #fig.text(0.75,0.7,'Updraft Speed (m s$^{-1}$)')
    plt.savefig(dir_prefix+'/plots/'+long_file_prefix+'_'+sc_name+'_xsect.png')
    plt.close()
