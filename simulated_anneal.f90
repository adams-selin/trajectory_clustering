! Note by RAS (30 Sept 2020) - all code/documentation below borrowed as-is from
! https://jblevins.org/mirror/amiller/simann.f90. Only additions were
! adding in variables necessary for the calc_Neps calculations in the
! entropy function call (previously FCN in original code.)
! RAS change 15 Oct 2020 - when generating a new xp (or epsilon, for clustering),
! sometimes it can be out-of-bounds (<0). Previous code picked a random 
! number between lb and ub, which isn't helpful if those values cover a 
! wide range. I changed code to randomly generate new xp's until it 
! gets one within the appropriate range.
! RAS change 24 Feb 2021 - changed all write statements to write to file 50
! (opened by prep_cluster where sa is called). Icky, I know.
...
! Latest revision of Fortran 90 version - 2 October 2013
    ...
    SUBROUTINE sa(n, x, max, rt, eps, ns, nt, neps, maxevl, lb, ub, c, iprint,  &
                  ...
                  distance, num_segments, return_opt_MinLns)
    !  Version: 3.2
    !  Date: 1/22/94.
    ...
    !  Required Subroutines (included, except FCN which is in its own file):
    ...
    !       cause any problems.
        ...
        INTEGER, INTENT(IN)       :: n, ns, nt, neps, maxevl
        INTEGER, INTENT(IN)       :: iprint, iseed1, iseed2
        INTEGER, INTENT(OUT)      :: nacc, nfcnev, nobds, ier
        LOGICAL, INTENT(IN)       :: max
        real, intent(in)          :: distance(:)
        integer(8), intent(in)    :: num_segments
        real, intent(out)         :: return_opt_MinLns
        ...
        IF (t <= 0.0) THEN
          WRITE(50,'(/, "  THE INITIAL TEMPERATURE IS NOT POSITIVE. "/,  &
        ...
        !print *, ' sa: initial entropy call'
        CALL entropy(n,x,f,distance,num_segments,opt_MinLns)!fcn(n, x, f)
        !print *, ' sa: after call n, x, f: ', n, x, f
        ...
        !  RAS Note - This was the original code, which basically yanks xp
        !  off to a random number anywhere between lb and ub.
        ...
        !        END IF
        !      END DO
        !  I would rather it just keep cycling within the vm range until it gets
        !  one in bounds, within a reasonable number of repetitions.
                do while ( (xp(i) .lt. lb(i)) .OR. (xp(i) .gt. ub(i)) )
                    xp(i) = x(i) + (ranmar()*2. - 1.) * vm(i)
                    lnobds = lnobds + 1
                    nobds = nobds + 1
                    IF(iprint >= 3) CALL prt3(max, n, xp, x, f)
        ...
        !  Evaluate the function with the trial point XP and return as FP.
              WRITE(50,'("  m, j, xp, vm: ",4G15.7)') m, j, xp, vm
              CALL entropy(n, xp, fp,distance,num_segments,opt_MinLns) !fcn(n, xp, fp)
              IF(.NOT. max) fp = -fp
              nfcnev = nfcnev + 1
              IF(iprint >= 3) CALL prt4(max, n, xp, x, fp, f)
              WRITE(50,'("    xp, x, fp, optMinLns: ",4G15.7)') xp, &
                x, fp, opt_MinLns
    ...
    !                                                      0 <= KL <= 30081
        ...
        IF( ij < 0  .OR.  ij > 31328  .OR. kl < 0  .OR.  kl > 30081 ) THEN
          WRITE(50, '(A)') ' The first random number seed must have a value ', &
                       'between 0 AND 31328'
          WRITE(50, '(A)') ' The second seed must have a value b/w 0 and 30081'
          STOP
        END IF
        i = MOD(ij/177, 177) + 2
        j = MOD(ij    , 177) + 2
    ...
    !  it minimizes by maximizing the negative a function.
        ...
        WRITE(50, '(/, "  THE STARTING VALUE (X) IS OUTSIDE THE BOUNDS "/,  &
    ...
    SUBROUTINE prt3(max, n, xp, x, f)
        ...
        IF (max) THEN
          WRITE(50, '("  CURRENT F: ", G25.18)') f
        ELSE
          WRITE(50, '("  CURRENT F: ", G25.18)') -f
        END IF
        CALL prtvec(xp, n, 'TRIAL X')
        WRITE(50, '("  POINT REJECTED SINCE OUT OF BOUNDS")')
    ...
    SUBROUTINE prt4(max, n, xp, x, fp, f)
        ...
        IF (max) THEN
          WRITE(50,'("  CURRENT F: ",G25.18)') f
          CALL prtvec(xp,n,'TRIAL X')
          WRITE(50,'("  RESULTING F: ",G25.18)') fp
        ELSE
          WRITE(50,'("  CURRENT F: ",G25.18)') -f
          CALL prtvec(xp,n,'TRIAL X')
          WRITE(50,'("  RESULTING F: ",G25.18)') -fp
    ...
    SUBROUTINE prt5()
        ...
        WRITE(50, '(/, "  TOO MANY FUNCTION EVALUATIONS; CONSIDER "/,  &
    ...
    LOGICAL, INTENT(IN) ::  max
        ...
        IF (max) THEN
          WRITE(50,'("  THOUGH LOWER, POINT ACCEPTED")')
        ELSE
          WRITE(50,'("  THOUGH HIGHER, POINT ACCEPTED")')
        END IF
    ...
    LOGICAL, INTENT(IN) :: max
        ...
        IF (max) THEN
          WRITE(50,'("  LOWER POINT REJECTED")')
        ELSE
          WRITE(50,'("  HIGHER POINT REJECTED")')
        END IF
    ...
    SUBROUTINE prt8(n, vm, xopt, x)
        ...
        WRITE(50,'(/, " intermediate results after step length adjustment", /)')
    ...
    SUBROUTINE prt9(max, n, t, xopt, vm, fopt, nup, ndown, nrej, &
        lnobds, nnew)
        ...
        WRITE(50,'(/," intermediate results before next temperature reduction",/)')
        WRITE(50,'("  CURRENT TEMPERATURE:            ",G12.5)') t
        IF (max) THEN
          WRITE(50, '("  MAX FUNCTION VALUE SO FAR:  ",G25.18)') fopt
          WRITE(50, '("  TOTAL MOVES:                ",I8)') totmov
          WRITE(50, '("     UPHILL:                  ",I8)') nup
          WRITE(50, '("     ACCEPTED DOWNHILL:       ",I8)') ndown
          WRITE(50, '("     REJECTED DOWNHILL:       ",I8)') nrej
          WRITE(50, '("  OUT OF BOUNDS TRIALS:       ",I8)') lnobds
          WRITE(50, '("  NEW MAXIMA THIS TEMPERATURE:",I8)') nnew
        ELSE
          WRITE(50, '("  MIN FUNCTION VALUE SO FAR:  ",G25.18)') -fopt
          WRITE(50, '("  TOTAL MOVES:                ",I8)') totmov
          WRITE(50, '("     DOWNHILL:                ",I8)')  nup
          WRITE(50, '("     ACCEPTED UPHILL:         ",I8)')  ndown
          WRITE(50, '("     REJECTED UPHILL:         ",I8)')  nrej
          WRITE(50, '("  TRIALS OUT OF BOUNDS:       ",I8)')  lnobds
          WRITE(50, '("  NEW MINIMA THIS TEMPERATURE:",I8)')  nnew
        END IF
        CALL prtvec(xopt, n, 'CURRENT OPTIMAL X')
    ...
    SUBROUTINE prt10()
        ...
        WRITE(50, '(/, "  SA ACHIEVED TERMINATION CRITERIA. IER = 0. ",/)')
    ...
    !  elements in VECTOR, 10 elements will be printed on each line.
        ...
        IF (ncols > 10) THEN
          ...
          DO i = 1, lines
            ll = 10*(i - 1)
            WRITE(50,1000) vector(1+ll:10+ll)
          END DO

          WRITE(50,1000) vector(11+ll:ncols)
        ELSE
          WRITE(50,1000) vector(1:ncols)
        END IF
